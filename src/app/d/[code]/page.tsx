...
export default function DiagnosticLinkPage() {
  ...
  const startDiagnostic = async () => {
    ...
    try {
      ...
      setError("That link is invalid or has expired. Ask for a new link and try again.");
      setStarting(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-white flex items-center justify-center px-4 py-12">
      <Card className="max-w-md w-full animate-fade-in-up">
        ...
        <CardContent className="space-y-5">
          ...
          >
            {starting ? "Starting..." : "I'm Ready - Let's Go!"}
            {!starting && <ArrowRight className="h-5 w-5 ml-2" />}
          </Button>

          {error && (
            <p className="text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg p-3" aria-live="polite">
              {error}
            </p>
          )}
          <p className="text-xs text-gray-400 text-center">This private link expires after seven days.</p>
        </CardContent>
      </Card>
    </div>
  );
